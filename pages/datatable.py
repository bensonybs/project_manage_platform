...
table_data_style = {"whiteSpace": "normal", "height": "auto", "font-family": "cursive"}
...
class Utility:
    ...
    def createForm(self, columns, values):
        ...
        for column in columns:
            title = dbc.Col(dbc.Label(children=column, html_for=column), width="2")
            # Shit code to assgin input form type, need to be refactor
            if column == "id":
                input = dbc.Col(
                    dcc.Markdown(id=column, children=f"系統自帶id_{values[column]}"),
                    width="6",
                )
            elif column == "公司":
                input = dbc.Col(
                ...
                input = dbc.Col(
                    dcc.Input(
                        id=column, type="number", step=1, min=0, value=values[column]
                    ),
                    width="6",
                )
            elif column == "ERP立案案號":
                input = dbc.Col(
                    dcc.Input(id=column, type="text", value=values[column]), width="6"
                )
            elif column == "預完日":
                input = dbc.Col(
                ...
                input = dbc.Col(
                    dcc.Input(
                        id=column, type="number", step=1, min=2000, value=values[column]
                    ),
                    width="6",
                )
                ...
                input = dbc.Col(
                    dbc.Textarea(id=column, value=values[column]), width="6"
                )
...
# Dash components
page_header = dbc.Row(
    dbc.Col(dcc.Markdown(children="### 碳排減量案件彙總")), style={"potision": "sticky"}
)
# Control Panel
page_size_dropdown = dbc.Col(
    dcc.Dropdown(
        placeholder="顯示案件筆數",
...
control_panel = html.Div(
    [
        dbc.Row(
            [
                dbc.Col(
                    dbc.Button(children="新增案件", color="success", id="create-button"),
                    width="auto",
                ),
                dbc.Col(
                    dbc.Button(
...
description_modal = dbc.Modal(
    ...
    4. 可直接在表格中修改資料，修改完畢後請點擊 **儲存**，若直接離開此頁則資料不會儲存
    """
                )
            ]
        ),
...
table = dbc.Row(
    dbc.Col(
        dash_table.DataTable(
            ...
            style_data=table_data_style,  # overflow cells' content into multiple lines
...
@callback(
    ...
    [Input("create-button", "n_clicks"), Input("data-table", "is_focused")],
    [State("data-table", "active_cell"), State("data-table", "data")],
...
@callback(
    [Output("description-modal", "is_open"), Output("description-button", "n_clicks")],
...
# Dash layout
layout = dbc.Container([page_header, control_panel, modal_pannel, table], fluid=True)
